...
<!-- action buttons -->
        <div v-if="actionsVisible" class="flex flex-col items-center">
            <button @click="emitWithdrawItem" class="rounded-md w-16 mt-1 px-1 py-1 bg-slate-300 border border-black text-sm font-medium hover:bg-slate-500 hover:text-white hover:border-2">
                Withdraw</button>
...
// to show/hide item details
    ...
    function emitWithdrawItem() {
        emit('emitWithdrawItem', props.id);
    }
