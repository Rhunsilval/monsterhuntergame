...
<!-- inventory -->
        <div class="col-span-3">
            ...
            <!-- <p>Swap Hold Item:</p>
            <ul>
                <li v-for="thing in playerStore.swapHolder" :key="thing.id">{{ thing.name }} </li>
            </ul> -->

            <div class="">
                <ul class="text-center grid grid-cols-10">
                    <li v-for="(item, index) in playerStore.playerPacked" :key="item">
                        ...
                        <!-- action buttons -->
                            <div v-if="actionsVisible" class="flex flex-col items-center">
                                <button @click="equipItem(item.id)" class="rounded-md w-16 mt-1 px-1 py-1 bg-slate-300 border border-black text-sm font-medium hover:bg-slate-500 hover:text-white hover:border-2">
                                    Equip</button>
                                ...
                                <button class="rounded-md w-16 mt-3 px-1 py-1 bg-slate-300 border border-black text-sm font-medium hover:bg-slate-500 hover:text-white hover:border-2">
                                    Use</button>
                                <button @click="dropItem(item.id)" class="rounded-md w-16 mt-3 px-1 py-1 bg-slate-300 border border-black text-sm font-medium hover:bg-slate-500 hover:text-white hover:border-2">
                                    Drop</button>
...
<script setup>
    ...
    // to know which item is being selected in the rendered playerPacked array
    ...
    // to make item actions list visible/invisible
    ...
    function equipItem(id) {
        ...
        let x = playerStore.playerPacked.findIndex(item => item.id === id);    
        ...
        if (chosenItem.value.itemSlot === 'null') {
            ...
            if (chosenItem.value.itemSlot === 'horse_shoes') {
                ...
                // if (playerStore.playerEquipped.player_boots.id === 'boots') {
                    equipBoots();
                    // playerStore.playerPacked.splice(x, 1);
                // } 
                // else {
                //     playerStore.swapHolder[1] = playerStore.playerEquipped.player_boots;
                //     playerStore.swapHolder.push(playerStore.playerEquipped.player_boots);
                //     equipBoots();
                //     playerStore.playerPacked.splice(x, 1);
                //     playerStore.playerPacked[x] = playerStore.swapHolder[1];
                // }
    ...
    function equipBoots() {
                ...
                playerStore.playerEquipped.player_boots.itemSlot = chosenItem.value.itemSlot;
                playerStore.playerEquipped.player_boots.itemShop = chosenItem.value.itemShop;
...
// to allow a player to drop an item from inventory
